...
export function AdminCafeConsole() {
  ...
  const fetchCafeProducts = async () => {
    ...
    try {
      ...
      // Filter only Cafe, Beverages, and Ice Cream items
      const cafeList = (data.products || []).filter((p: any) => {
      ...
      setProducts(cafeList)
    } catch (err) {
      toast.error('Failed to load Cafe catalog')
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="space-y-6">
      {/* Console Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-amber-500/10 via-orange-500/5 to-transparent p-6 rounded-3xl border border-border/40">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-xl bg-orange-500/10 text-orange-500 flex items-center justify-center">
              <Coffee className="h-4.5 w-4.5" />
            </div>
            <h2 className="text-xl font-black text-text-primary">FastKirana Cafe Chef Console</h2>
          </div>
          <p className="text-xs text-text-secondary">
            Manage live menu availability, chef stock counters, and pricing for Cafe snacks, brews, ice creams, and beverages.
          </p>
        </div>
        
        <button
          onClick={fetchCafeProducts}
          disabled={loading}
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
        <div className="relative w-full sm:max-w-md">
          ...
          <input
            type="text"
            placeholder="Search Cafe dishes, ice creams, beverages..."
            value={searchQuery}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-xs text-text-secondary font-bold">Syncing Cafe kitchen items...</p>
        </div>
      ) : filteredList.length === 0 ? (
        <div className="text-center py-16 bg-muted/10 rounded-3xl border border-dashed border-border/60">
          <Coffee className="h-10 w-10 text-text-muted mx-auto mb-2.5 opacity-60" />
                ...
                <div>
                  <div className="flex gap-3">
                    <div className="h-14 w-14 shrink-0 rounded-2xl bg-muted overflow-hidden relative border border-border/40">
                      {product.imageUrl ? (
                        ...
                        <div className="h-full w-full flex items-center justify-center text-xl bg-gradient-to-tr from-orange-100 to-amber-50 dark:from-zinc-900 dark:to-zinc-800">
                          🍳
                        </div>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-4">
                    <div className="bg-muted/30 p-2 rounded-2xl border border-border/30 text-center space-y-1">
                      <span className="text-[9px] font-extrabold uppercase tracking-wider text-text-secondary">Chef Counter</span>
