...
export default async function OrderTrackingPage({ params }: OrderTrackingPageProps) {
  ...
  const order = {
    ...
    address: {
      label: orderRaw.address?.label || 'Pickup Location',
      houseNo: orderRaw.address?.houseNo || '',
      street: orderRaw.address?.street || '',
      area: orderRaw.address?.area || 'Hub Store',
      city: orderRaw.address?.city || 'Kanpur',
      pincode: orderRaw.address?.pincode || '209206',
      lat: orderRaw.address?.lat || 26.1534185,
      lng: orderRaw.address?.lng || 80.1714024,
    },
