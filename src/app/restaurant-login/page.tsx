...
import { Loader2, UtensilsCrossed, KeyRound, Mail, ArrowLeft } from 'lucide-react'
...
export default function RestaurantLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAllowed) {
        toast.success('Welcome back, Chef! Redirecting to kitchen...')
        router.push('/restaurant-kitchen')
        router.refresh()
      } else {
        toast.error('Access Denied. Only registered Wedson Restaurant chefs are allowed to log in here.')
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-4 sm:px-6 relative overflow-hidden">
      {/* Culinary background glow */}
      <div className="absolute top-1/4 left-1/4 h-[350px] w-[350px] rounded-full bg-red-600/10 blur-[120px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 h-[350px] w-[350px] rounded-full bg-amber-500/5 blur-[120px] pointer-events-none" />
      ...
      <div className="w-full max-w-md space-y-6 z-10">
        ...
        {/* Login Card */}
        <div className="bg-[#111113] border border-zinc-800/60 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-red-950/5">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled={isLoading}
                  required
                  className="w-full bg-zinc-900 border border-zinc-800 focus:border-red-500/50 pl-10 pr-4 py-2.5 rounded-2xl text-xs font-semibold text-white focus:outline-none placeholder-zinc-600 disabled:opacity-50"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full inline-flex items-center justify-center gap-2 py-3 bg-red-600 hover:bg-red-700 text-white font-black text-xs rounded-2xl shadow-lg shadow-red-600/10 active:scale-98 transition-all cursor-pointer disabled:opacity-60"
            >
              {isLoading ? (
                <>
          ...
          <p className="text-[9px] font-bold text-center text-zinc-500 mt-6 uppercase tracking-wider leading-relaxed">
            Restricted Area • Authorized Kitchen Personnel Only
          </p>
        </div>
      </div>
    </div>
