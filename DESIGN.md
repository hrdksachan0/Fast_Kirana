...
FastKirana is an ultra-fast quick-commerce grocery and food delivery platform combining fast delivery with high visual polish. The atmosphere is crisp, vibrant, and clinical yet warm. It features an asymmetric dual-personality interface — **Grocery Mode** (energetic red & savings green) and **Cafe Mode** (warm culinary orange).
...
- **Tabular Numerals / Price Tags:** `JetBrains Mono` / `Inter` (`font-variant-numeric: tabular-nums`) — Price numbers (`₹49`), timer displays (`02:45:30`), and order IDs.
- **Banned:** Generic serif fonts (`Times New Roman`, `Georgia`), pure black text (`#000000`).

---

## 4. Component Stylings
- **Header & Delivery Badge:** Top bar features live location ("Deliver to Home • Ghatampur Market") with an animated ⚡ express badge pill in translucent green (`rgba(0,177,64,0.1)`).
...
- **Micro-Animations:** Pulsing delivery badge, sliding banner carousel, and cart badge bounce.
